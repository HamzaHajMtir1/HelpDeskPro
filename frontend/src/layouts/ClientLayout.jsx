/**
 * ClientLayout.jsx — avec avatar dynamique dans le header
 * ✅ Avatar synchronisé via useAvatar hook
 * ✅ Clic sur le cercle/nom → /profile
...
import ClientHelpPanel from '../components/Help/ClientHelpPanel';
...
export default function ClientLayout({ children }) {
  ...
  const navItems = [
    ...
    { icon: PlusCircle,      label: 'Créer un ticket',       path: '/tickets/new' },
    { icon: Ticket,          label: 'Mes tickets',           path: '/tickets'     },
    { icon: BookOpen,        label: 'Base de connaissances', path: '/knowledge'   },
  ...
  return (
    <div className="flex h-screen" style={{ backgroundColor: '#f3f4f6', overflow: 'clip' }}>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs mt-1 ml-3" style={{ color: '#6b6b6b' }}>
                {role === 'TECHNICIEN' ? 'Espace Technicien' : 'Portail Client'}
              </p>
            </div>
      ...
      <div className="flex-1 flex flex-col min-w-0" style={{ overflow: 'hidden' }}>
        ...
        {/* ✅ FIX dropdown : overflow-y scroll mais pas clip sur x */}
        <main
      ...
      {/* Panels flottants */}
      <ClientHelpPanel isOpen={helpOpen} onClose={() => setHelpOpen(false)} />
      <ChatbotWidget />
    </div>
  );
}
