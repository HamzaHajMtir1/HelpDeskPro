/**
 * AdminLayout.jsx — avec avatar dynamique dans le header
 * ✅ Avatar se met à jour instantanément après changement dans ProfilePage
 * ✅ Clic sur le cercle → navigation vers /admin/profile
 * ✅ useAvatar hook pour synchronisation cross-composant
...
import {
  LayoutDashboard, Ticket, Clock, BookOpen,
  BarChart2, Users, Settings, LogOut, Menu, X,
  User, PlusCircle, UserPlus, HelpCircle
} from 'lucide-react';
import NotificationDropdown from '../components/NotificationDropdown';
import { useSettings } from '../context/SettingsContext';
import AdminHelpPanel from '../components/Help/AdminHelpPanel';
...
export default function AdminLayout({ children }) {
  ...
  const navItems = [
    ...
    { icon: PlusCircle,      label: 'Créer un ticket',      path: '/admin/new'             },
    { icon: Ticket,          label: 'Gestion des tickets',  path: '/admin/tickets'         },
    { icon: Clock,           label: 'Gestion SLA',          path: '/admin/sla'             },
    { icon: BookOpen,        label: 'Base de connaissances',path: '/admin/adminknowledge'  },
    { icon: BarChart2,       label: 'Rapports',             path: '/admin/reports'         },
    { icon: Users,           label: 'Utilisateurs',         path: '/admin/users'           },
    { icon: UserPlus,        label: 'Demandes de comptes',  path: '/admin/account-requests'},
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ backgroundColor: '#f3f4f6' }}>
      ...
      {/* Panel d'aide admin */}
      <AdminHelpPanel isOpen={helpOpen} onClose={() => setHelpOpen(false)} />
    </div>
  );
}

/* ─── Sous-composant : cercle avatar réutilisable ─── */
